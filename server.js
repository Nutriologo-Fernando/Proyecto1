...
app.post('/api/orders', async (req, res) => {
    ...
    for (const item of cart) {
        ...
        if (Number(item.price) !== product.price) {
            return res.status(400).json({
                error: `Precio inválido para ${item.name}. Esperado: ${product.price}, Recibido: ${item.price}`
            });
        }
    ...
    try {
        ...
        await db.guardarOrden(orderData);
...
app.post('/api/orders/:orderId/capture', async (req, res) => {
    ...
    try {
        ...
        if (orderDetails.result.status === 'COMPLETED') {
            console.log("✅ Pago ya capturado:", orderDetails.result);
            return res.status(200).json(orderDetails.result);
        }
